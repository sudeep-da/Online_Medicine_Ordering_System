...
<div class="card card-outline card-info">
  ...
  <div class="card-body">
    <ul>
    <?php
      $customers = $conn->query("SELECT id, CONCAT(firstname, ' ', lastname) as name FROM customer_list");
      while($row = $customers->fetch_assoc()):
    ?>
      <li><a href="index.php?customer_id=<?= $row['id'] ?>"><?= htmlspecialchars($row['name']) ?></a></li>
    <?php endwhile; ?>
    </ul>
  </div>
</div>
...
<?php if (isset($_GET['customer_id'])): 
  ...
  $orders = $conn->query("SELECT * FROM order_list WHERE customer_id = $customer_id ORDER BY id DESC");
?>
<div class="card card-outline card-secondary mt-4">
  <div class="card-header">
    <h3 class="card-title">Orders for Customer ID: <?= $customer_id ?></h3>
  </div>
  <div class="card-body">
